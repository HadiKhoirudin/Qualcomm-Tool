...
::: Partition FRP -> Persistent
for /f "delims= " %%c in ('type %cache%\partition^|find "persistent"') do (
  set "line=%%c"
  set "line=!line:*frp =!
  set /a "result_frp=!line:~1!" 2>nul
)
IF "%result_frp%" == "1" (for /F "Tokens=7 " %%d in ('findstr /I "persistent" %cache%\partition') do (echo.Partition FRP Sector       : %%d)
	%sleep% 1
	%emmcdl% -p %USBComPort% -f %Loader% -d persistent %backup_persistent%-persistent.img -memoryname %MemoryName% >nul
